package com.trading.TradingUpFundationBackend.commons.constant.URLs;
/**
 * Interface who contains the endpoints for each controller for the entity UserTrading
 */
public interface IUserTradingEndPoints {
    String URL_USER = "/user";//Go to de user area
    String URL_USER_CREATE = "/user_create";//Create a user area
    String URL_USER_DELETE = "/user_delete";//Delete a user area
    String URL_USER_UPDATE = "/user_update";//Update a user area
    String URL_USER_READ = "/user_read{id}";//Search a user area
    String URL_USERS_READ = "/users_read";//Search all the users area
    String URL_USER_LOGIN = "/login";
}
