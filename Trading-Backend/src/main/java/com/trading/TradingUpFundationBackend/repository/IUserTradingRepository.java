...
public interface IUserTradingRepository extends JpaRepository<UserTradingEntity, Integer> {
     ...
     * @param email The email of the user who is gonna be searched
     * @return An object empty or not, depends if it found the email or not
     */
    Optional<UserTradingEntity> findByEmail(String email);

    /**
     * Class without body which delete an user for his email
